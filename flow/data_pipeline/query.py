...
class QueryStrings(Enum):
    ...
    POWER_DEMAND_MODEL = "SELECT id, time, speed, acceleration, 1200 * speed * " \
                         ...
                         "* POW(speed,3) AS power, 'POWER_DEMAND_MODEL' AS energy_model_id, source_id " \
    ...
    POWER_DEMAND_MODEL_DENOISED_ACCEL = \
                         ...
                         "((CASE WHEN accel_without_noise > 0 THEN 1 ELSE 0 END * (1-0.8) * accel_without_noise)+0.8 " \
                         "+ 9.81 * SIN(road_grade)) + 1200 * 9.81 * 0.005 * speed + 0.5 * 1.225 * 2.6 * 0.3 " \
                         "* POW(speed,3) AS power, 'POWER_DEMAND_MODEL_DENOISED_ACCEL' AS energy_model_id, source_id " \
                         "FROM trajectory_table " \
                         "WHERE partition_name=\'{partition}\' " \
                         "ORDER BY id, time "
