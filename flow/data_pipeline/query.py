...
class QueryStrings(Enum):
    ...
    "WITH sub1 AS ( " \
        "SELECT" \
        "time, id, speed, acceleration, accel_without_noise, road_grade, source_id," \
        "time - LAG(time, 1) " \
        ...
        ")," \
        "sub2 AS (" \
        "SELECT time, id, speed, acceleration, accel_without_noise, " \
        "road_grade, source_id, " \
        "speed-prev_accel*sim_step+prev_accel_denoised*sim_step AS speed_denoised" \
        "FROM sub1" \
        ") " \
