...
class MergeGenerator(Generator):
    ...
    def specify_nodes(self, net_params):
        ...
        nodes = [
            {"id": "inflow_highway",
             "x": repr(-INFLOW_EDGE_LEN), "y": repr(0)},
            {"id": "left", "y": repr(0), "x": repr(0)},
            {"id": "center", "y": repr(0), "x": repr(premerge)},
            {"id": "right", "y": repr(0), "x": repr(premerge+postmerge)},
            {"id": "inflow_merge",
    ...
    def specify_edges(self, net_params):
        ...
        edges = [
            {"id": "inflow_highway", "type": "highwayType",
             "from": "inflow_highway", "to": "left",
             "length": repr(INFLOW_EDGE_LEN)},
            {"id": "left", "type": "highwayType",
             "from": "left", "to": "center", "length": repr(premerge)},
            {"id": "inflow_merge", "type": "mergeType",
             "from": "inflow_merge", "to": "bottom",
             "length": repr(INFLOW_EDGE_LEN)},
            {"id": "bottom", "type": "mergeType",
             "from": "bottom", "to": "center", "length": repr(merge)},
            {"id": "center", "type": "highwayType",
             "from": "center", "to": "right", "length": repr(postmerge)}
        ]
    ...
    def specify_types(self, net_params):
        ...
        types = [{"id": "highwayType", "numLanes": repr(h_lanes),
                  "speed": repr(speed)},
                 {"id": "mergeType", "numLanes": repr(m_lanes),
                  "speed": repr(speed)}]

        return types

    def specify_routes(self, net_params):
        rts = {"inflow_highway": ["inflow_highway", "left", "center"],
               "left": ["left", "center"],
               "center": ["center"],
