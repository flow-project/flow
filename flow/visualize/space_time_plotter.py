...
def emission_space_time_plot(data, title='Space-time Plot',
                             filename="space_time_plot",
                             save=False, show=True):
    cdict = {'red': ((0., 0., 0.), (0.2, 1., 1.), (0.6, 1., 1.), (1., 0., 0.)),
             'green': ((0., 0., 0.), (0.2, 0., 0.), (0.6, 1., 1.),
                       (1., 1., 1.)),
             'blue': ((0., 0., 0.), (0.2, 0., 0.), (0.6, 0., 0.),
                      (1., 0., 0.))}
    ...
    for car in np.unique(data['id']):
        ...
        disc_cond = np.logical_or(np.abs(np.diff(pos)) >= 5,
                                  abs(np.diff(lane)) > 0.01)
...
if __name__ == "__main__":
    data = pd.read_csv(
        '/Users/nishant/Development/research/frankencistar/rllabcistar/'
        'test_time_rollout/two-lane-stabilization-rl.csv')
