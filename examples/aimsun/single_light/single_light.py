...
ADDITIONAL_ENV_PARAMS = {'target_nodes': [3369],
...
class SingleLightEnv(Env):
    def __init__(self, env_params, sim_params, network, simulator='aimsun'):
        ...
        self.current_phase = np.zeros(len(self.target_nodes))
    ...
    def observation_space(self):
        ...
        shape = len(self.target_nodes)*ap['num_incoming_edges_per_node']\
            * (ap['num_stopbars']+ap['num_advanced'])*ap['num_measures']*[len(ap['target_nodes'])]
    ...
    def _apply_rl_actions(self, rl_actions):
        ...
        """delta_phase = actions - self.current_phase
        for node_id, action in zip(self.target_nodes, delta_phase):
            if action:
                self.k.traffic_light.change_to_next_phase(node_id, action)
        self.current_phase = actions"""
        for node_id, action in zip(self.target_nodes, actions):
            if action:
    ...
    def get_state(self, rl_id=None, **kwargs):
        ...
        num_phase = len(self.target_nodes)

        shape = (num_nodes, num_edges, num_detectors_types, num_measures, num_phase)
        ...
        for i, node_id in enumerate(self.target_nodes):
            curr_phase = self.k.traffic_light.___
            state[(*k, 2)] = curr_phase

        ##Also add state of neighbor traffic lights
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        print(f'{self.k.simulation.time:.0f}', '\t', f'{reward:.2f}', '\t', self.current_phase.flatten())
