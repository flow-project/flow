...
def twolanemergestraight(sumo_binary=None):
    ...
    additional_env_params = {"target_velocity": 40, "num_steps": 1000}
    env_params = EnvParams(additional_params=additional_env_params,
                           lane_change_duration=1)
    # inflow = InFlows()
    # inflow.add(veh_type="lane_0_human", edge="1", period=1.0,
    #            departLane=0, departSpeed=1)
    # inflow.add(veh_type="lane_1_human", edge="1", period=1.0,
    #            departLane=1, departSpeed=1)

    net_params = NetParams(no_internal_links=False)
    print([("1", i) for i in range(10, 100, 9)] + [("1", i) for i in range(10, 100, 9)])
    initial_config = InitialConfig(positions=[("1", i) for i in range(10, 200, 40)] + [("1", i) for i in range(10, 200, 40)],
                                   lanes=["0"]*9 + ["1"]*9)
