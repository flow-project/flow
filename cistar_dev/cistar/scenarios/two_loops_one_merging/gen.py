...
class TwoLoopOneMergingGenerator(Generator):
    ...
    def specify_nodes(self, net_params):
        ...
        nodes = [{"id": "bottom", "x": repr(0),                           "y": repr(-r * sin(angle_small))},
                 {"id": "right",  "x": repr(r * (1 + cos(angle_large))),  "y": repr(0)},
                 {"id": "top",    "x": repr(0),                           "y": repr(r * sin(angle_small))},
                 {"id": "left",   "x": repr(-r * (1 + cos(angle_small))), "y": repr(0)}]
    ...
    def specify_edges(self, net_params):
        ...
        edges = [{"id": "center", "type": "edgeType",
                  "from": "bottom", "to": "top", "length": repr(ring_edgelen),
                  "shape": " ".join(["%.2f,%.2f" % (r * (cos(t) - cos(angle_small)), r * sin(t))
                                     for t in linspace(- pi/3, pi/3, resolution)])},

                 {"id": "right_top", "type": "edgeType",
                  "from": "right", "to": "top", "length": repr(merge_edgelen),
                  "shape": " ".join(["%.2f,%.2f" % (1.5*r * (cos(t) + cos(angle_large)), 1.5*r * sin(t))
                                     for t in linspace(0, pi - angle_large, resolution)])},

                 {"id": "right_bottom", "type": "edgeType",
                  "from": "bottom", "to": "right", "length": repr(merge_edgelen),
                  "shape": " ".join(["%.2f,%.2f" % (1.5*r * (cos(t) + cos(angle_large)), 1.5*r * sin(t))
                                     for t in linspace(pi + angle_large, 2 * pi, resolution)])},
    ...
    def specify_routes(self, net_params):
        ...
        See parent class
        """
        rts = {"right_top": ["right_top", "left_top", "left_bottom", "right_bottom", "right_top"],
               "right_bottom": ["right_bottom", "right_top", "left_top", "left_bottom", "right_bottom"],
               "left_top": ["left_top", "left_bottom", "center", "left_top"],
               "left_bottom": ["left_bottom", "center", "left_top", "left_bottom"],
