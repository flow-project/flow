class BaseLaneChangingController:
    ...
    def __init__(self, veh_id, lane_change_params):
        ...
        # min_gap defines the minimum gap (in meters) that a car is willing to accept
        # in front and behind it to enter a gap
    ...
    def get_safe_lane_change_action(self, env, target_lane):
        """
        Determines whether a collision will occur if a vehicle enters the target lane.
        ...
        # if no lane change is being performed, there is no need to check for safety
        if current_lane == target_lane:
            return target_lane

        # if the target lane is not within the range of lanes available, return the current lane
        ...
        # if there is only one vehicle in the environment, or there are no vehicles in the target
        ...
        max_trail_vel = trail_vel + env.env_params.get_additional_param["max-acc"] * time_step
        max_this_vel = this_vel + env.env_params.get_additional_param["max-acc"] * time_step

        # if vehicle may collide into a vehicle in the target lane in the lane change
        # is performed, opt out of lane change
